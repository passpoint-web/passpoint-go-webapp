...
const WalletTable = ({ wallet, currency, styles, updateKey, allRendered }) => {
  ...
  const transactionTypes = ["All", "Incoming", "Outgoing", "Payment"]
  ...
  const getAllTransactions = async (
    ...
    try {
      const response = await wallet.allTransactions({
        data: filters,
        type:
          type === "Incoming"
            ? "collection"
                    ...
                    {transactions.map((data, id) => (
                      <tr key={id}>
                        <td className={tableStyles.td_4}>
                          {
                            <div className={tableStyles.col}>
                              <h4>
                                {data.beneficiaryAccountName?.length > 20
                                  ? `${data.beneficiaryAccountName.substring(
                                      0,
                                      18
                                    )}...`
                              ...
                              "PFL0000000" ? (
                                <div
                                  className={tableStyles.accountNum}
                                  style={{ display: "flex", gap: 10 }}
                                >
                                  <p>
                                    {data.beneficiaryWalletId ||
                                      data.beneficiaryAccountNumber}
                                  </p>
                                  <CopyValue
                                    color="#009ec4"
                        ...
                        <td className={tableStyles.td_2}>
                          {
                            <div
                              className={`${data.transactionStatus?.toLowerCase()}-tag`}
                            >
