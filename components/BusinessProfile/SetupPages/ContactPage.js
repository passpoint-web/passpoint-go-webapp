...
const ContactPage = ({ styles }) => {
  ...
  const [atleastTwoSocialsProvided, setAtleastTwoSocialsProvided] =
    useState(false);
  ...
  const handleSocialsChange = (value, name) => {
    ...
    setAtleastTwoSocialsProvided(
      socials.filter((s) => isValidUrl(s.url)).length > 1
    );
    setSocials(update);
  };
    ...
    <form onSubmit={handleSubmit}>
      ...
      >
        <PhoneInput
          ...
          onChange={(phone) =>
            handleChange({ target: { name: "companyPhone", value: phone } })
          }
        />
      </Input>
      <Input
      ...
      <FormChoice
        message="Do you want to have web-contact form?"
        checkValue={payload.hasWebContact}
        onChange={() => {
          handleChange({
            target: {
              name: "hasWebContact",
              value: payload.hasWebContact === 1 ? 0 : 1,
            },
          });
        }}
      />
  ...
  const SocialsForm = () => (
    <form onSubmit={handleFinalSubmit}>
      ...
      <div
        style={
          socials.length
            ? { borderTop: "1px solid #e5e7ef", paddingTop: "32px" }
            : {}
        }
      >
        {socials.map((s, id) => (
          <div key={id} style={{ position: "relative" }}>
            ...
            <Input
              ...
              onChange={(e) =>
                handleSocialsChange(e.target.value, e.target.name)
              }
            />
            <button
              title="Delete"
      ...
      {finalCtaClicked && !atleastTwoSocialsProvided ? (
        <FeedbackInfo message="Need atleast two of your social media links" />
      ) : (
        <></>
      )}
      <div className={AuthStyles.action_ctn}>
  ...
  return (
    <>
      ...
      <div className={styles.inner}>
        <BackBtn
          onClick={() => push("/dashboard/business-profile-setup/services")}
        />
        ...
        {contactLevel ? ContactsForm() : SocialsForm()}
      </div>
    </>
  );
};
