...
const BusinessPage = ({ styles }) => {
  ...
  const editFeature = () => {
    ...
    const update = features.map((oldFeature) =>
      oldFeature.id === currentEditId ? feature : oldFeature
    );
    setFeatures(update);
    // reset state
    setShowModal(false);
  ...
  const AddBusinessFeatures = () => (
    <div className={styles.features_ctn}>
      ...
      {features
        .filter((f) => f.headline && f.description)
        .map((feat, id) => (
          <FeatureCard
            key={id}
    ...
    >
      <div>
        ...
        <Input
          label="Description"
          error={
            (modalCtaClicked && !feature.description) ||
            feature.description.length > 200
          }
          errorMsg={
            modalCtaClicked && !feature.description
              ? "Description of feature is required"
              : feature.description.length > 200
        ...
        >
          ...
          />
          {feature.description.length <= 200 ? (
            <FeedbackInfo message="Maximum of 200 characters" type="note" />
          ) : (
            <></>
          )}
        </Input>
  ...
  return (
    <>
      ...
      <div className={styles.inner}>
        <BackBtn
          onClick={() => push("/dashboard/public-profile-setup/identity")}
        />
